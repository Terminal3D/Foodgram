...
class RecipeManager:
    ...
    @staticmethod
    def add_recipe_to_collection(user, recipe_id, collection_model, serializer_class, request):
        try:
            recipe = Recipe.objects.get(pk=recipe_id)
        except Recipe.DoesNotExist:
            return Response({'error': 'Рецепт не найден.'}, status=http.HTTPStatus.BAD_REQUEST)

...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def download_shopping_cart(self, request, pk=None, *args, **kwargs):
        ...
        try:
            ...
            return Response({'error': 'Корзина покупок не найдена.'}, status=http.HTTPStatus.NOT_FOUND)

        recipes = shopping_cart.recipes.all()
        if not recipes.exists():
            return Response({'error': 'Ваша корзина пуста.'}, status=http.HTTPStatus.OK)
...
class SubscriptionsListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        try:
            return Subscription.objects.get(user=self.request.user).subscription.all()
...
class SubscriptionsAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({'error': 'Пользователь не найден.'}, status=http.HTTPStatus.NOT_FOUND)
